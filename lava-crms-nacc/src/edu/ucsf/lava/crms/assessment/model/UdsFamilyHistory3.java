...
public class UdsFamilyHistory3 extends UdsInstrument{
  ...
  public void setKid15ago(Short kid15ago) {
		...
		if(version.equalsIgnoreCase("3")){
			if (this.getPacket() != null) {
				if (this.getPacket().equals("I")){
					this.nwinfmut = this.nwinfpar = this.nwinfsib = this.nwinfkid = (short)-8;
				}
				if (this.getPacket().equals("F") || getPacket().equals("T")){
					this.afffamm = (short)-8;
				}
			}
		}
	}
	...
	public String[] getRequiredResultFields() {
      ...
      "dadageo",
      "sibs",
      // technique is to enable the first field for each sibling (and child), which is the Month of Birth mob field, instead of having
      // complex skip logic to enable/disable siblings (children) based on the number of siblings (children) input by the user. The
      // mob field in turn has skip logic which can enable the rest of the fields in the row when mob has a valid value
      //"sib1mob",
      "sib1yob",
      "sib1agd",
      "sib1neu",
	...
	if (this.getPacket().equals("I")){
		...
		required = StringUtils.concatenateStringArrays(required, new String[]{"afffamm"});
	}
	if (this.getPacket().equals("F") || getPacket().equals("T")){
		// add these to required
